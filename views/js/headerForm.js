...
const searchIcon = document.querySelector(".searchBtnMobile .fa");
...
window.onclick = async function (event) {
  ...
  if (e == checkoutFormWrapper) {
    checkoutForm.style.display = "none";
  }
  if (e == loginFormWrapper || e == loginForm) {
    loginForm.style.display = "none";
  }
  ...
  if (
    ...
    if (e != searchIcon && e != searchBox) {
      searchBox.style.display = "none";
      searchBtn.style.display = "none";
      searchBtnMobile.style.display = "inline-block";
    }
  }
};
...
function createProduct(product) {
  ...
  const qtyText = createElement("div");
  qtyText.classList.add("mobile-text");
  ...
  selectList.addEventListener("change", () => {
    ...
    const subtotal = (
      ...
      parseInt(productCart.children[i].children[2].textContent.split(".")[1])
    ).toString();
  ...
  priceText.classList.add("mobile-text");
  priceText.textContent = "價格";
  price.appendChild(priceText);
  price.append(`NT.${product.price}`);

  const subtotal = createElement("div");
  subtotal.classList.add("subtotal");

  const subtotalText = createElement("div");
  subtotalText.classList.add("mobile-text");
  subtotalText.textContent = "小計";
  subtotal.appendChild(subtotalText);
  subtotal.append(`NT.${product.subtotal}`);
...
function updateTotal() {
  const total = parseInt(
    productList.map((product) => product.subtotal).reduce((a, b) => a + b, 0)
  );
